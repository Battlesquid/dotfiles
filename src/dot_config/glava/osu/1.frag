...
// this applies a color to a fragment
#if USE_ALPHA > 0
...
void test(float section, float center, float theta, float d, float scalar, vec4 color) {
    ...
    if (abs(ym) < BAR_WIDTH / 2) {                 /* if within width, draw audio */
        ...
        float v;
        if (idx > 0) {
            v = smooth_f(audio_l);/* left buffer */
        } else {
            v = smooth_f(audio_r); /* right buffer */
        }
        v *= (AMPLIFY * scalar);                                                  /* amplify */
        #undef smooth_f
...
void main() {
	...
	// the outline has an inner and outer edge, so we divide by two and then add/subtract for each
	// respective edge
    ...
    if (d > C_RADIUS) {
        ...
        const float center = ((TWOPI / NBARS) / 2.0F); /* center line angle */
				...
				test(section, center, theta + 2.5, d, 1, vec4(0.3F, 0.3F, 0.3F, 0.02F));
				test(section, center, theta + 6.5, d, 1.5, vec4(0.7F, 0.7F, 0.7F, 0.02F));
				test(section, center, theta + 4.5, d, 1.7, vec4(0.5F, 0.5F, 0.5F, 0.02F));
				test(section, center, theta, d, 0.7, vec4(0.75F, 0.75F, 0.75F, 0.02F));
    }
}
